...
def replace_wan_attention(
    ...
    if pattern == "SVG":
        ...
        # SSS: Selective Step Skipping
        logger.info(
            f"Configuring SSS attention with warmup_steps: {warmup_steps}, "
            f"num_keyframes: {num_keyframes}, skip_n: {skip_n}, "
        ...
        AttnModule = WanAttn_SSSAttn_Processor

        # Reset runtime state for new inference
        AttnModule.keyframe_indices = None
        AttnModule.step_counter = 0
        AttnModule.last_timestep = None
        AttnModule.current_branch = 0
        AttnModule.branch_counter = 0

        # Set SSS-specific parameters
        AttnModule.warmup_steps = warmup_steps
        AttnModule.num_keyframes = num_keyframes
        AttnModule.skip_n = skip_n
        AttnModule.similarity_window = similarity_window
        AttnModule.keyframe_strategy = keyframe_strategy
        AttnModule.keyframe_similarity_threshold = keyframe_similarity_threshold
        AttnModule.min_keyframe_gap = min_keyframe_gap
        AttnModule.sss_schedule = sss_schedule
        AttnModule.sss_min_skip = sss_min_skip
        AttnModule.sss_max_skip = sss_max_skip
        AttnModule.sss_transition_points = sss_transition_points
        AttnModule.total_inference_steps = total_inference_steps

        AttnModule.first_layers_fp = first_layers_fp
        AttnModule.first_times_fp = first_times_fp
        AttnModule.logging_file = logging_file

        # These might be needed by the processor
        AttnModule.context_length = context_length
        AttnModule.num_frame = num_frame_patches
        AttnModule.frame_size = frame_patches_one_frame

        # STEP 1.1: Prepare SVG sparse attention masks for SSS v2 optimization
        # Set use_svg_sparse=True via environment variable or future parameter
        use_svg_sparse = os.environ.get("SSS_USE_SVG_SPARSE", "false").lower() == "true"
        AttnModule.use_svg_sparse = use_svg_sparse

        if use_svg_sparse:
            logger.info("[SSS v2] Preparing SVG spatial sparse mask for denoise steps")

            # Reuse SVG's mask preparation logic
            multiplier = diag_width = sparsity_to_width(
                sparsity, context_length, num_frame_patches, frame_patches_one_frame
            )
            ...
            if attention_backend == "flexattn":
                ...
                AttnModule.svg_block_mask = block_mask
                logger.info(f"[SSS v2] SVG block_mask prepared: {block_mask}")
